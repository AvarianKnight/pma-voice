...
AddEventHandler('pma-voice:syncRadioData', function(radioTable)
	...
	for tgt, enabled in pairs(radioTable) do
		if tgt ~= playerServerId then toggleVoice(tgt, enabled) end
	end
...
AddEventHandler('pma-voice:removePlayerFromRadio', function(plySource)
	if plySource == playerServerId then
		for tgt, enabled in pairs(radioData) do
			if tgt ~= playerServerId then toggleVoice(tgt, false) end
		end
		radioData = {}
		playerTargets(radioData, callData)
	else
...
function setRadioChannel(channel)
	...
	if GetConvarInt('voice_enableUi', 1) == 1 then
		SendNUIMessage({ radioChannel = channel, radioEnabled = Cfg.radioEnabled })
	end
end

exports('setRadioChannel', setRadioChannel)
exports('removePlayerFromRadio', function() setRadioChannel(0) end)
exports('addPlayerToRadio', function(radio)
	local radio = tonumber(radio)
	if radio then setRadioChannel(radio) end
end)
...
RegisterCommand('+radiotalk', function()
	...
	if GetResourceState("pma-ambulance") ~= "missing" then
		if exports["pma-ambulance"]:isPlayerDead() then return false end
...
RegisterCommand('-radiotalk', function()
	if voiceData.radio > 0 or Cfg.radioEnabled then
		...
		TriggerEvent("pma-voice:radioActive", false) playMicClicks(false)
...
AddEventHandler('pma-voice:clSetPlayerRadio', function(radioChannel) voiceData.radio = radioChannel end)