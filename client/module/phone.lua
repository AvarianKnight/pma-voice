	...
	Citizen.CreateThread(function()
		...
		while voiceData.call ~= 0 do
			...
			if NetworkIsPlayerTalking(PlayerId()) and not changed then
				changed = true TriggerServerEvent('pma-voice:setTalkingOnCall', true)
			elseif changed and NetworkIsPlayerTalking(PlayerId()) ~= 1 then
				changed = false TriggerServerEvent('pma-voice:setTalkingOnCall', false)
			end
			Wait(0)
		end
	end)
...
function setCallChannel(channel)
	...
	if GetConvarInt('voice_enableUi', 1) == 1 then SendNUIMessage({ callInfo = channel }) end
	createPhoneThread()
end
