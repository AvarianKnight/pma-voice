...
local function getGridZone()
    ...
    return (math.floor(31 * ( --[[ offset from the original zone should return a multiple]] zoneOffset) +
        --[[ returns -6 * zoneOffset so we want to offset it ]]
                           (zoneOffset * 6) - 6)) +
               ( --[[ Offset routing bucket by 5 (we listen to closest 5 channels) + 5 (routing starts at 0)]] ((LocalPlayer.state
                   .routingBucket or 0) * 5) + 5) + math.ceil((plyPos.x + plyPos.y) / (zoneRadius))
end
...
Citizen.CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/mute', 'Mutes the player with the specified id', {{
        name = "player id",
        help = "the player to toggle mute"
    }})
    while true do
        if MumbleIsConnected() then
            updateZone()
            if GetConvarInt('voice_enableUi', 1) == 1 then
                if lastRadioStatus ~= voiceData.radioPressed or lastTalkingStatus ~=
                    (NetworkIsPlayerTalking(PlayerId()) == 1) then
        ...
        if GetConvar('voice_externalAddress', '') ~= externalAddress or GetConvarInt('voice_externalPort', 0) ~=
            externalPort then
